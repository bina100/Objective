...
export default class BtnAdmin extends React.Component{
    constructor(props){
        ...
        this.pushStudentsOrGuidesOrDirectoresToDB = this.pushStudentsOrGuidesOrDirectoresToDB.bind(this)
    ...
    clickDeleteDirector(){
        ...
        this.setState({show_add_guides:false})
        this.setState({show_add_students:false})
        this.setState({show_add_directiones:false})
        this.setState({show_add_questionnaire:false})
        this.setState({show_add_labs:false})
        this.setState({show_add_scheduling:false})
        this.setState({show_delete_student:false})
    ...
    loadStudentsOrGuidesOrDirector=(e)=>{
    ...
    pushStudentsOrGuidesOrDirectoresToDB(e){
        if(e.target.id == "students"){
            ...
            (async ()=> {
                ...
                if(response.data.code === 'ER_DUP_ENTRY')//making sure not loading doubles
                    alert('duplicates in director file')
                else if(response.data === 'Failed')
                    alert("error loading directores")
                else{
                    alert("directores loaded")
                    this.setState({show_add_directores:false})
                }
            })();
        }
        ...
        (async ()=> {
            const response = await axios.post(
              ...
              if(response.data === 'Failed')
                ...
                this.setState({show_delete_guide:false})
                this.setState({show_delete_student:true})
                console.log("abc ", this.user_type)
                if(this.user_type == "students")
                    this.setState({succeed_delete_s:true})
                else if (this.user_type == "guides")
        ...
        return(
            <div dir="rtl">
                ...
                {this.state.show_add_students && <div>
                        ...
                        <input type="file" accept=".xlsx" id="students" onChange={this.loadStudentsOrGuidesOrDirector}/>
                        <button id="students" onClick={this.pushStudentsOrGuidesOrDirectoresToDB.bind(this)}>טעינה</button>
                        </div>
                        <div className="chosen-window">
                ...
                {this.state.show_add_guides && <div className="chosen-window">
                        ...
                        <input type="file" accept=".xlsx" id="guides" onChange={this.loadStudentsOrGuidesOrDirector}/>
                        <button id="guides" onClick={this.pushStudentsOrGuidesOrDirectoresToDB.bind(this)}>טעינה</button>
                    </div>}  
                {this.state.show_add_directores && <div className="chosen-window">
                    {/* <div><button onClick={(e)=> this.setState({show_add_directores:false})}>x</button></div> */}
                    <p/><label>בחר קובץ לטעינת מנהל מעבדה </label><p/>
                    <input type="file" accept=".xlsx" id="directiones" onChange={this.loadStudentsOrGuidesOrDirector}/>
                    <button id="directiones" onClick={this.pushStudentsOrGuidesOrDirectoresToDB.bind(this)}>טעינה</button>
                </div>}
